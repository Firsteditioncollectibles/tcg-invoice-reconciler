...
- This is a client-only TCGplayer reconciler. Keep file processing local and validate required details and amounts before export. The user explicitly requested removal of the PDF title and repeated buyer-prepared disclaimer blocks on 20 September 2026. The later explicit exact-layout request also removes the added total/footer/page-number labels: PDFs and their previews follow the supplied marketplace screenshot, including unboxed metadata columns, the source header buttons and four-column table. Keep reconciliation identified in the app and PDF metadata; do not represent the output as newly seller-issued.
...
- Latest user preference: replace left-hand product thumbnails with one standard small Pokemon card-back image in every row, in the editor and PDF. This supersedes the earlier thumbnail-matching requirement. Existing drafts may retain source crops, but render the bundled card back instead.

- 21 September: user requested fewer clicks. Remove per-row review toggles, mark-all and final confirmation; valid orders can preview/download directly. Keep automatic validation, extraction warnings and stale-preview invalidation. Name PDF/draft downloads from the order seller (Shipped and sold by), falling back to order number. The legacy reviewed flag remains only for draft compatibility, not export gating.

- User clarified filenames must be only the Shipped and sold by store name plus `.pdf` (or `.json` for drafts), with no added reconciled/draft labels. Keep reconciliation identity in the app/PDF metadata.
