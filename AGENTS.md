...
- This is a client-only TCGplayer reconciler. Keep file processing local and require review before export. The user explicitly requested removal of the PDF title and repeated buyer-prepared disclaimer blocks on 20 September 2026. The later explicit exact-layout request also removes the added total/footer/page-number labels: PDFs and their previews follow the supplied marketplace screenshot, including unboxed metadata columns, the source header buttons and four-column table. Keep reconciliation identified in the app, download filename, PDF metadata; do not represent the output as newly seller-issued.
