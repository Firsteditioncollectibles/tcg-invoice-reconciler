...
- Browser tests now cover direct export, invalid data gating, old draft reopening, stale-preview invalidation, and exact store-based PDF/draft filenames. GitHub Actions **35566628139** passed typecheck, all 24 unit/PDF tests, build and all 13 browser workflows on `09106cdd6307569e38b27470baecb39103163e79`. Preview smoke verified zero checkboxes, direct export available on a valid order, invalid quantity blocking export, store-only filename `Sample Card House.pdf`, and actual PDF preview. Published as `bb8e3449d820c1725d670e006ca728ac08a8cfa3`, production deployment `dpl_62gRESNNqsnTn26ebRdsooxKjYpA` READY. Fresh public-site smoke confirmed zero review checkboxes, enabled download on a valid example, and store-only filename `Sample Card House.pdf`. Evidence: `docs/evidence/direct-export-live-20260921.jpg`. Save a draft before refreshing an old tab.
...
- On 21 September the user reported the live site still showed original thumbnails. Confirmed the tested change existed only on `feat/pokemon-card-backs`; production still pointed to `73f0925`. Published as `39ac4e74fa2345f758fa19fb060f4e4e072b3890`: production deployment `dpl_DPZtHhPRebVLm6phYchR6tM2dRY8` is READY at https://tcg-invoice-reconciler.vercel.app/. A fresh public-site PDF upload yielded 13 identical embedded JPEG card backs, visually verified in the editor; evidence: `docs/evidence/card-backs-live-20260921.jpg`. Existing open tabs need a refresh; save a draft first to preserve edits. No CardScout resources are involved.
